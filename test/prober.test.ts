...
describe("generateProbeCandidates", () => {
  it("strips common query params", () => {
    const candidates = generateProbeCandidates(
      "https://example.com/image.jpg?w=400&h=300&q=80",
    );
  ...
  it("tries larger dimensions", () => {
    ...
    expect(candidates.some((candidate) => candidate.includes("w=2560"))).toBe(
      true,
    );
  });
  ...
  it("tries master and original path variants", () => {
    ...
    expect(candidates.some((candidate) => candidate.includes("/master/"))).toBe(
      true,
    );
    expect(
      candidates.some((candidate) => candidate.includes("/original/")),
    ).toBe(true);
  });
});
