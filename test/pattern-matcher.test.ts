...
import { applyCuratedPattern, applyPattern, matchCuratedPattern } from "../src/pattern-matcher";
...
describe("matchCuratedPattern", () => {
  ...
  it("does not match host-like pattern domains inside unrelated hostnames", () => {
    expect(
      matchCuratedPattern("https://res.cloudinary.com.evil.test/demo/image/upload/w_400/file.jpg"),
    ).toBeNull();
  });
  ...
  it("unwraps SpeedSize image proxy URLs to original source images", () => {
    ...
    ).toBe("https://www.tilebar.com/media/catalog/product/lines-black-tile.jpg");
  });

  it("strips empty numeric cachebuster query params with the generic pattern", () => {
    expect(
      applyCuratedPattern("https://www.tilebar.com/media/wysiwyg/Vinyl.png?1=", "generic"),
