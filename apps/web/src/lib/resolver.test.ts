...
describe("resolve", () => {
  ...
  it("uses curated pattern if matched", async () => {
    ...
    vi.mocked(matchCuratedPattern).mockReturnValue(
      "https://source.com/image.jpg"
    );
    vi.mocked(validateImageUrl).mockResolvedValue({
      valid: true,
      size: 100_000,
    });
  ...
  it("uses learned pattern if curated pattern not found", async () => {
    const { getCached } = await import("./cache-repo");
    const { matchCuratedPattern, applyPattern } = await import(
      "./pattern-matcher"
    );
    const { validateImageUrl } = await import("./validator");
    const { findPatternByDomain, incrementSuccess } = await import(
      "./patterns-repo"
    );
    ...
    vi.mocked(applyPattern).mockReturnValue(
  ...
  it("calculates size increase correctly", async () => {
    ...
    vi.mocked(validateImageUrl)
      .mockResolvedValueOnce({ valid: true, size: 50_000 }) // original
      .mockResolvedValueOnce({ valid: true, size: 200_000 }); // resolved
  ...
  it("continues when database pattern lookup fails", async () => {
    ...
    vi.mocked(findPatternByDomain).mockRejectedValue(
